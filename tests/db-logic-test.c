...
void check_list( gchar *error_prefix, gchar **orig, gchar **list )
{
  ...
  if( sum != 0 ) g_error( "%s: wrong list", error_prefix );

}


// Функция сверяет список проектов с контрольным списком.
void check_project_list( HyScanDB *db, gchar *error_prefix, gchar **orig )
{
  ...
  if( list == NULL ) g_error( "%s: can't get project list", error_prefix );
...
void check_track_list( HyScanDB *db, gchar *error_prefix, gchar **orig, guint32 project_id )
{

  gchar **list;

  // Получаем список галсов в проекте.
...
void check_channel_list( HyScanDB *db, gchar *error_prefix, gchar **orig, gint32 track_id )
{

  gchar **list;

  // Получаем список каналов данных галса.
  list = hyscan_db_get_channel_list( db, track_id );
  if( list == NULL ) g_error( "%s: can't get channel list", error_prefix );
...
void check_project_param_list( HyScanDB *db, gchar *error_prefix, gchar **orig, guint32 project_id )
{

  gchar **list;

  // Получаем список групп параметров проекта.
  list = hyscan_db_get_project_param_list( db, project_id );
  if( list == NULL ) g_error( "%s: can't get project group parameter list", error_prefix );
...
void check_track_param_list( HyScanDB *db, gchar *error_prefix, gchar **orig, guint32 track_id )
{

  gchar **list;

  // Получаем список групп параметров галса.
  list = hyscan_db_get_track_param_list( db, track_id );
  if( list == NULL ) g_error( "%s: can't get track group parameter list", error_prefix );
...
void check_param_list( HyScanDB *db, gchar *error_prefix, gchar **orig, guint32 param_id )
{
  ...
  if( list == NULL ) g_error( "%s: can't get parameter list", error_prefix );
...
void set_parameters( HyScanDB *db, gchar *error_prefix, gint value, guint32 param_id )
{
...
void check_parameters( HyScanDB *db, gchar *error_prefix, gint value, guint32 param_id )
{
  ...
  if( g_strcmp0( svalue, orig_svalue ) != 0 ) g_error( "%s: error in string parameter", error_prefix );
...
// Функция копирует в группу параметров еще 4.
void add_temporary_parameters( HyScanDB *db, gchar *error_prefix, gint value, guint32 project_id, guint32 param_id )
{

  gint src_param_id = hyscan_db_open_project_param( db, project_id, "src-temp" );
  if( src_param_id < 0 ) g_error( "%s: can't create temporary parameters group", error_prefix );
...
int main( int argc, char **argv )
{
  ...
  gchar *params[] = { "default.integer", "default.double", "common.boolean", "common.string", NULL };

  // Контрольный список расширенных параметров в группах.
  gchar *params_ex[] = { "default.integer", "default.double", "common.boolean", "common.string",
                         "test1.param1", "test1.param2", "test2.param1", "test2.param2", NULL };
  ...
  if( n_projects < 2 ) n_projects = 2;
  if( n_tracks < 2 ) n_tracks = 2;
  if( n_channels < 2 ) n_channels = 2;
  if( n_gparams < 2 ) n_gparams = 2;
  ...
  for( i = 0; i < n_projects; i++ ) projects[i] = g_strdup_printf( "Project %d", i + 1 );
  for( i = 0; i < n_tracks; i++ ) tracks[i] = g_strdup_printf( "Track %d", i + 1 );
  for( i = 0; i < n_channels; i++ ) channels[i] = g_strdup_printf( "Channel %d", i + 1 );
  for( i = 0; i < n_gparams; i++ ) gparams[i] = g_strdup_printf( "Parameters %d", i + 1 );
  ...
  for( i = 1; i < n_projects; i += 2 ) projects2[ i / 2 ] = g_strdup_printf( "Project %d", i + 1 );
  for( i = 1; i < n_tracks; i += 2 ) tracks2[ i / 2 ] = g_strdup_printf( "Track %d", i + 1 );
  for( i = 1; i < n_channels; i +=2 ) channels2[ i / 2 ] = g_strdup_printf( "Channel %d", i + 1 );
  for( i = 1; i < n_gparams; i += 2 ) gparams2[ i / 2 ] = g_strdup_printf( "Parameters %d", i + 1 );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_gparams; j++ )
      {
      gchar *error_prefix = g_strdup_printf( "add_temporary_parameters( '%s.%s' )", projects[i], gparams[j] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_gparams; j++ )
      {
      gchar *error_prefix = g_strdup_printf( "check_temporary_parameters( '%s.%s' )", projects[i], gparams[j] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_gparams; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "set_parameters( '%s.%s.%s' )", projects[i], tracks[j], gparams[k] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_gparams; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "check_parameters( '%s.%s.%s' )", projects[i], tracks[j], gparams[k] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_gparams; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "check_param_list( '%s.%s.%s' )", projects[i], tracks[j], gparams[k] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_gparams; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "add_temporary_parameters( '%s.%s.%s' )", projects[i], tracks[j], gparams[k] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_gparams; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "check_temporary_parameters( '%s.%s.%s' )", projects[i], tracks[j], gparams[k] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_channels; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "check_param_list( '%s.%s.%s' )", projects[i], tracks[j], channels[k] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_channels; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "add_temporary_parameters( '%s.%s.%s' )", projects[i], tracks[j], channels[k] );
  ...
  for( i = 0; i < n_projects; i++ )
    for( j = 0; j < n_tracks; j++ )
      for( k = 0; k < n_channels; k++ )
        {
        gchar *error_prefix = g_strdup_printf( "check_temporary_parameters( '%s.%s.%s' )", projects[i], tracks[j], channels[k] );
